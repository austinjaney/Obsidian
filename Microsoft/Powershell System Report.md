...
# This Powershell reporting script was created by Sean Duffy @ https://www.red-gate.com/simple-talk/sysadmin/powershell/building-a-daily-systems-report-email-with-powershell/
# Austin has modified it to work well with PDQ deploy (or as a stand alone script that can be run on any system)
# the origional script called for for a list of computers to be used this one will grab the current computers name and run using that.

### Dependencies (PIL Customizations)
# This script as written requires that we own the ozprinciple.net domain and the iti@ozprinciple.net email
# it also requires that http://www.w3.org/TR/html4/frameset.dtd exists

### Notes
# This script was written to provide a daily status update on our internal servers, we acomplish this threw a scedualed task in PDQ deploy but this script could just as easily be set to run threw
# a task scedualer. This script does contain the login creditenals for an email account iti@ozprinciple.net, this is not a production domain that
#


#region Variables and Arguments
$users = "iti@ozprinciple.com" # List of users to email your report to (separate by comma)
$fromemail = "iti@ozprinciple.net"
$server = "smtp.ozprinciple.net" #enter your own SMTP server DNS name / IP address here
$sendingemail = “iti@ozprinciple.net”
$PWord = ConvertTo-SecureString –String “9n#*K8hCZH8f” –AsPlainText -Force
$Creds = New-Object –TypeName System.Management.Automation.PSCredential –ArgumentList $sendingemail, $pword
$list = $args[0] #This accepts the argument you add to your scheduled task for the list of servers. i.e. list.txt
$computers = $env:COMPUTERNAME #grab the names of the servers/computers to check from the list.txt file.
...
send-mailmessage -port 587 -from $fromemail -to $users -subject "$env:ComputerName Systems Report" -Attachments $ListOfAttachments -BodyAsHTML -body $HTMLmessage -priority Normal -smtpServer $server -Credential $creds
